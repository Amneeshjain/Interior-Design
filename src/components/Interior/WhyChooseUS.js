...
const WhyChooseUS = () => {
  const points = [
    ...
    },
    {
      ...
      description:
        "Vestibulum quis nisi condimentum at ac nec ornare morbi in ridiculus ipsum faucibus in proin ante sagittis blandit interdum quis mauris, at pharetra quam aliquet donec orci viverra id.",
    },
    {
  ...
  return (
    <div className={`container ${style.relative}`}>
      ...
      <div className={`row`}>
        <h1
          className={`col-lg-6 ${styles.fs56} ${styles.fw400} ${styles.uppercase} d-flex align-items-center`}
        >
      ...
      <div
        className={`row ${style.flexChoose} ${styles.marginNagative}  ${style.absoluteChoose}`}
      >
          ...
          <div className={`row ${style.flexChoose}`}>
            {points.map((point, index) => (
              <div className="col-lg-9" key={index}>
                <div
                  className={`row ${index % 2 === 0 ? style.even : style.odd}`}
                >
                  <div className="col-lg-6">
                    <h3 className={style.stepTitle}>{point.title}</h3>
                    <p className={`${style.stepDescription} ${styles.fs18}`}>
                      {point.description}
                    </p>
                  </div>
                </div>
